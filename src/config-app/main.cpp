...
void ShowCorrectionHelpDialog(HWND hwndDlg, int typingMode) {
    if (typingMode == 0) { // Vietnamese
        std::wstring text =
            ...
            L"   - Dùng TỪ ĐỨNG TRƯỚC để chọn khi một lỗi gõ có nhiều cách hiểu:\n"
            L"     gõ nhầm \"phood\" sau chữ \"thành\" ra \"phố\", vì \"thành phố\"\n"
            L"     là cụm có thật còn \"thành phổ\" thì không. Chỉ xét khi nhấn\n"
            L"     phím cách, và chỉ khi đúng một cách hiểu hợp với từ trước;\n"
            L"     không rõ thì Neokey giữ nguyên như bạn đã gõ.\n\n"
        ...
        std::wstring text =
            ...
            L"   - Uses the WORD BEFORE to choose when a typo has several readings:\n"
            L"     a slipped \"phood\" after \"thành\" becomes \"phố\", because\n"
            L"     \"thành phố\" is a recorded pair and \"thành phổ\" is not. Only on\n"
            L"     Space, and only when exactly one reading fits the previous word;\n"
            L"     anything less certain is left exactly as you typed it.\n\n"
...
void TranslateDialog(HWND hwndDlg, int typingMode) {
    if (typingMode == 0) { // Vietnamese
        ...
        SetDlgItemTextW(hwndDlg, IDC_CHECK_AUTO_SYNTHETIC_FALLBACK,
                        L"Thử đổi cách gõ khi ứng dụng hiện ô nhập nhỏ");
        ...
        SetDlgItemTextW(hwndDlg, IDC_CHECK_AUTO_SYNTHETIC_FALLBACK,
                        L"Try another typing mode when an app shows a floating input box");
        ...
        case WM_INITDIALOG: {
            ...
            CheckDlgButton(
                ...
                config.enable_auto_word_segmentation
                    ...
                    : BST_UNCHECKED);
            CheckDlgButton(
                hwndDlg, IDC_CHECK_AUTO_SYNTHETIC_FALLBACK,
                config.enable_auto_synthetic_fallback
                    ? BST_CHECKED
                    : BST_UNCHECKED);
